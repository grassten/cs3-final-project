...
public class BinarySearchTree<T> implements BSTInterface<T>
{
   ...
   private List<T> recGetAll(T target, BSTNode<T> node) {
      if (node == null) {
         return resultSet;
      } else if (comp.compare(target, node.getInfo()) < 0) {
         return recGetAll(target, node.getLeft());
      } else if (comp.compare(target, node.getInfo()) > 0) {
         return recGetAll(target, node.getRight());
      } else {
         resultSet.add(node.getInfo());
         return recGetAll(target, node.getLeft());
      }
   }

   public List<T> getAll(T target) {
      resultSet.clear();
      return recGetAll(target, root);
   }
